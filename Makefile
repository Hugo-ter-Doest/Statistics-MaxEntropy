# This Makefile is for the Statistics::MaxEntropy extension to perl.
#
...
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     EXE_FILES => [q[ME.wrapper.pl]]
#     NAME => q[Statistics::MaxEntropy]
#     PREREQ_PM => {  }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[MaxEntropy.pm]
...
NAME_SYM = Statistics_MaxEntropy
VERSION = 1.0
...
MAKE = make
FULLEXT = Statistics/MaxEntropy
BASEEXT = MaxEntropy
PARENT_NAME = Statistics
DLBASE = $(BASEEXT)
VERSION_FROM = MaxEntropy.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
O_FILES  = 
H_FILES  = 
MAN1PODS = ME.wrapper.pl
MAN3PODS = Candidates.pm \
	ME.wrapper.pl \
	MaxEntropy.pm \
	SparseVector.pm
...
TO_INST_PM = Candidates.pm \
	ME.wrapper.pl \
	MaxEntropy.pm \
	SparseVector.pm

PM_TO_BLIB = MaxEntropy.pm \
	$(INST_LIB)/Statistics/MaxEntropy.pm \
	Candidates.pm \
	$(INST_LIB)/Statistics/Candidates.pm \
	SparseVector.pm \
	$(INST_LIB)/Statistics/SparseVector.pm \
	ME.wrapper.pl \
	$(INST_LIB)/Statistics/ME.wrapper.pl
...
DISTNAME = Statistics-MaxEntropy
DISTVNAME = Statistics-MaxEntropy-1.0
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN1EXT) --perm_rw=$(PERM_RW) \
	  ME.wrapper.pl $(INST_MAN1DIR)/ME.wrapper.pl.$(MAN1EXT) 
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) \
	  Candidates.pm $(INST_MAN3DIR)/Statistics::Candidates.$(MAN3EXT) \
	  SparseVector.pm $(INST_MAN3DIR)/Statistics::SparseVector.$(MAN3EXT) \
	  ME.wrapper.pl $(INST_MAN3DIR)/Statistics::ME.wrapper.$(MAN3EXT) \
	  MaxEntropy.pm $(INST_MAN3DIR)/Statistics::MaxEntropy.$(MAN3EXT) 
...
EXE_FILES = ME.wrapper.pl

pure_all :: $(INST_SCRIPT)/ME.wrapper.pl
	$(NOECHO) $(NOOP)

realclean ::
	$(RM_F) \
	  $(INST_SCRIPT)/ME.wrapper.pl 

$(INST_SCRIPT)/ME.wrapper.pl : ME.wrapper.pl $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/ME.wrapper.pl
	$(CP) ME.wrapper.pl $(INST_SCRIPT)/ME.wrapper.pl
	$(FIXIN) $(INST_SCRIPT)/ME.wrapper.pl
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/ME.wrapper.pl
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name: Statistics-MaxEntropy' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Statistics-MaxEntropy",' >> META_new.json
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  MaxEntropy.pm $(INST_LIB)/Statistics/MaxEntropy.pm \
	  Candidates.pm $(INST_LIB)/Statistics/Candidates.pm \
	  SparseVector.pm $(INST_LIB)/Statistics/SparseVector.pm \
	  ME.wrapper.pl $(INST_LIB)/Statistics/ME.wrapper.pl 
